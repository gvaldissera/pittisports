...
function desenharProdutoNoCarrinho(idProduto) {
    ...
    const cartaoProdutoCarrinho = `<button id="remover-item-${produto.id}" class="absolute top-1 right-2">
      <i class="fa-solid fa-rectangle-xmark text-stone-400 hover:text-stone-950"></i>
    </button>        
  ...
  <div class="flex text-stone-950 items-end absolute bottom-0 right-2 text-lg">
        ...
        <button class="ml-1" id="incrementar-produto-${produto.id}">+</button>
  </div>`;
...
export function atualizarPrecoCarrinho() {
    ...
    precoCarrinho.innerText = `Subtotal: $${precoTotalCarrinho}`;
}